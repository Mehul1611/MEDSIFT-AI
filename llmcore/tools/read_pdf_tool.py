...
_READ_PDF_TOOL_DESCRIPTION = f'''Use this tool to read the raw text content from specific pages of a patient's clinical PDF document. 
Use it only when a page summary is unclear, when you need to verify, or when two summaries conflict and you need the original source text. 
Do not use this for every page — page-wise summaries are your primary source.'''
...
def get_read_pdf_tool(notes_dir: str) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        func=configured_fn,
        name=_READ_PDF_TOOL_NAME,
        description=_READ_PDF_TOOL_DESCRIPTION,
        args_schema=ReadPdfSchema,
    )
